# =========================
# copyright
# This file is taken from Nathan Stoikopoulos
# nathan.stoikopoulos@gmail.com
#
# This file was taken from the 'seg2_files' project, licensed under the
# terms of the MIT License.
#
# https://github.com/natstoik/seg2_files

from typing import Tuple
import struct
import numpy as np

from .segy import save_segy
...
def load_seg2(filename: str)-> Tuple[np.ndarray, dict]:
    """Load seg2 files
    ...
    header['dt'] = header['tr']['SAMPLE_INTERVAL'][0]
    header["sx"] = header['tr']['SOURCE_LOCATION']
    header['sy'] = np.zeros_like(header['tr']['SOURCE_LOCATION'])
    header["gx"] = header['tr']['RECEIVER_LOCATION']
    header['gy'] = np.zeros_like(header['tr']['RECEIVER_LOCATION'])
    header['offset'] = np.abs(header["sx"] - header["gx"])
    
    return trace_array, header


def seg2_segy(filename: str, 
              data: np.ndarray, 
              header: dict, 
              verbose: bool=False) -> None:
    """Save the outputs of load_seg2 in a segy file
    ...
    segy_header["samples"] = segy_header["dt"] * np.arange(data.shape[0])
...
if __name__ == "__main__":
    pass
    # You can use this file as here
    # data, header = load_seg2(data_file)